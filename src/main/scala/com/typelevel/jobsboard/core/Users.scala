...
trait Users[F[_]] {
  ...
  def delete(userEmail: String): F[Boolean]
}
...
final class LiveUsers[F[_]: MonadCancelThrow: Logger] private (xa: Transactor[F]) extends Users[F] {
  ...
  override def delete(userEmail: String): F[Boolean] =
    sql"DELETE FROM users WHERE email = $userEmail".update.run.transact(xa).map(_ > 0)
}
